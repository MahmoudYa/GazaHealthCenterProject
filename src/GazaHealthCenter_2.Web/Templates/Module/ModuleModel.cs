...
public class ModuleModel
{
    ...
    public ModuleModel(String model, String controller, String? area)
    {
        ...
        Type modelType = assembly.GetType($"GazaHealthCenter_2.Objects.{model}") ?? typeof(AModel);
        Type viewType = assembly.GetType($"GazaHealthCenter_2.Objects.{model}View") ?? typeof(AView<>);
        BindingFlags declaredOnly = BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly;

        ControllerNamespace = $"GazaHealthCenter_2.Controllers.{area}".Trim('.');
        ...
        {
            Type type = assembly.GetType($"GazaHealthCenter_2.Objects.{model}{view}View") ?? viewType;
